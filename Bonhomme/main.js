...
class Figure {
  ...
  createEyes() {
    ...
    eyes.position.z = 0.7;
    eyes.position.y = -0.1; // Slightly below center for better placement
  }
...
window.addEventListener("keyup", (event) => {
	...
	if (event.code === "ArrowUp") {
		...
		gsap.to(
			[...figure.arms.map(a => a.rotation), ...figure.legs.map(l => l.rotation)],
...
blink(idleTimeline.to({}, { duration: 2 }))
  .to({}, { duration: 4.5 })
  .to(figure.head.rotation, { z: degreesToRadians(7),  duration: 0.7, ease: "power2.out" })
  .to(figure.head.rotation, { z: degreesToRadians(-7), duration: 0.7, ease: "power2.out" })
...
blink(idleTimeline)
  .to({}, { duration: 2.5 })
  .to(figure.head.rotation, { x: degreesToRadians(7), duration: 0.2, ease: "power2.out" })
...
const walkTimeline = gsap.timeline({ repeat: -1, paused: true })
  .to(figure.arms[0].rotation, { x: -walkAngle, duration: 0.3, ease: "sine.inOut" }, 0)
  .to(figure.arms[1].rotation, { x:  walkAngle, duration: 0.3, ease: "sine.inOut" }, 0)
  .to(figure.legs[0].rotation, { x:  walkAngle, duration: 0.3, ease: "sine.inOut" }, 0)
  .to(figure.legs[1].rotation, { x: -walkAngle, duration: 0.3, ease: "sine.inOut" }, 0)
  .to(figure.arms[0].rotation, { x:  walkAngle, duration: 0.3, ease: "sine.inOut" }, 0.3)
  .to(figure.arms[1].rotation, { x: -walkAngle, duration: 0.3, ease: "sine.inOut" }, 0.3)
  .to(figure.legs[0].rotation, { x: -walkAngle, duration: 0.3, ease: "sine.inOut" }, 0.3)
  .to(figure.legs[1].rotation, { x:  walkAngle, duration: 0.3, ease: "sine.inOut" }, 0.3);
...
ambientFolder.add(guiParams, "ambientIntensity", 0, 2, 0.1)
  .name("Intensité")
  .onChange((value) => {
...
directionalFolder.add(guiParams, "directionalIntensity", 0, 2, 0.1)
  .name("Intensité")
  .onChange((value) => {
    directionalLight.intensity = value;
  });

directionalFolder.add(directionalLight.position, "x", -20, 20, 0.5).name("Position X");
directionalFolder.add(directionalLight.position, "y", 0, 20, 0.5).name("Position Y");
directionalFolder.add(directionalLight.position, "z", -20, 20, 0.5).name("Position Z");

// Dossier pour les ombres
const shadowFolder = gui.addFolder("Ombres");
shadowFolder.add(guiParams, "showShadows")
...
helpersFolder.add(guiParams, "showLightHelper")
  .name("Helper lumière")
  .onChange((value) => {
    lightHelper.visible = value;
  });

helpersFolder.add(guiParams, "showArmHelpers")
  .name("Helpers bras")
  .onChange((value) => {
...
// Couleur de fond
gui.addColor(guiParams, "backgroundColor")
